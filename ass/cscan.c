...
void scan(int disk[],int limit,int head,int start,int finish){
	...
	if(upperLimit!=0)
		for(int  i=0;i<upperLimit;i++)
			printf("[%d]->",upper[i]);
	printf("[%d*]->[%d*]->",finish,start);
	for(int  i=0;i<lowerLimit-1;i++)
		printf("[%d]->",lower[i]);
	if(lowerLimit!=0)
		printf("[%d]\n",lower[lowerLimit-1]);
	totalMovement=lowerLimit!=0?(finish-initialHead)+finish-start+lower[lowerLimit-1]:finish-initialHead+finish-start;
